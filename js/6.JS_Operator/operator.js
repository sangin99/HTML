...
// 선대입 후증가
result = single++ * 8;   // 결과값 : 0
// 선증가 후대입
result = ++single * 8;  // 결과값 : 16 
// 선대입 후감소
result = single-- + 9 - ++single;  //  결과값 : 9
// 선감소 후대입
result = --single;
...
console.log(typeof+'10');  // (+)기호를 붙임으로 문자타입을 숫자타입으로 인식한다
console.log(+true);  // true = 1 / false = 0 으로 인식한다
...
/*
    할당 연산자 : 우항에 있는 피연산자와 좌항에 연산하여 좌항에 대입
    -  = : 좌항에 우항을 대입
...
const result2 = number % 2 ?  '홀수' : '짝수';             // 동일한 값을 가진다
console.log(result2)
...
//* 삼항 연산자로 표현해보기
const number = ''; 
const result3 = number > 0 ? '양수' : 
                            number < 0 ? '음수' : '영'   


/*
    논리 연산자 : 우황과 좌항의 피연산자를 논리 연산
...
console.log(false || '홍길동');      // 위에 식에서 "홍길동" = true 를 받아서, ( false || true ) 상황에서 true 를 예상하지만! 그에 할당된 값을 반환해준다!
console.log(true && '홍길동');     
console.log(false && '홍길동');

console.log('홍길동' && true);      // true .. 뒤의 값을 반환해준다
